...
int read_from(int client_index, struct sockname *usernames) {
	...
	if (usernames[client_index].username == NULL) {
		...
		//read(fd, usernames[client_index].username, BUF_SIZE);
	} else {
		buf[num_read] = '\0';
		char message[BUF_SIZE];
		// int buf_len = strlen(usernames[client_index].username);
		...
		for (int i = 0; i < MAX_CONNECTIONS; i++) {
			if (usernames[i].sock_fd > -1) {
				int w = write(usernames[i].sock_fd, message, strlen(message));
				if (num_read == 0 || w != strlen(message)) {
